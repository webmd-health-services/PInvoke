...
$zipFileName = $artifacts | Where-Object 'fileName' -Like 'PureInvoke*.zip' | Select-Object -ExpandProperty 'fileName'
if (-not $zipFileName)
{
    $msg = 'Unable to download PureInvoke ZIP file artifact because it does not exist.'
...
Rename-Item -Path 'PureInvoke' -NewName 'PureInvoke.orig'
